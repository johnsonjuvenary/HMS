 <?php
  if (isset($_POST['allocate_room_female'])) {
    ...
    if ($student_check == 1) {
      ...
      ";
        ...
        $room_check = mysqli_num_rows(mysqli_query($connection, "SELECT room_number,block_name FROM hostelers WHERE block_name = 'D' AND room_number = '$room_number'"));
        if ($room_check == 4) {
 ...
 <div class="card">
   <div class="card-body">
     <h5 class="text-dark text-center">Female Students Rooms Allocation</h5>
     <hr />
     <?php
      //returning students
      $students = mysqli_query($connection, "SELECT * FROM students INNER JOIN applicants ON students.regno = applicants.student_regno AND applicants.payment = 'paid' AND applicants.selected = 'selected' AND students.gender = 'female' AND students.regno NOT IN (SELECT student_regno FROM hostelers)");
      //returning rooms
      $rooms_blocks = mysqli_query($connection, "SELECT * FROM rooms JOIN blocks ON rooms.block_id = blocks.id AND block_name = 'D' ORDER BY room_number");
      ?>
     <div class="container">
       <form action="" method="POST" class="form-group">
         <div class="form-row">
           ...
           <div class="col-md-1">
             <input type="text" class="form-control" name="block_name" value="D" readonly>
           </div>
           <div class="col-md-4">
           ...
           <div class="col-md-3">
             <input type="submit" name="allocate_room_female" class="form-control btn btn-primary" value="Allocate Room">
           </div>
         </div>
       </form>
     </div>
